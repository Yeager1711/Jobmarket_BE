...
export class JobController {
        ...
        @Post('recruitment-PostJob')
        async postCompanyJob(@Req() req: Request, @Body() jobData: any) {
                try {
                        const recruitmentId = (req as any).user?.recruitmentId;
                        if (!recruitmentId) {
                                throw new HttpException(
                                        'Unauthorized: Missing recruitmentId',
                                        HttpStatus.UNAUTHORIZED
                                );
                        }

                        // Add recruitmentId to jobData for service to validate/use
                        jobData.recruitmentId = recruitmentId;
                        const savedJob = await this.jobService.postCompanyJob(jobData);
                        return {
                                success: true,
